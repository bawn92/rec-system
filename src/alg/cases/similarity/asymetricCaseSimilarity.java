...
public class asymetricCaseSimilarity implements CaseSimilarity
{
	...
	final static double MEAN_WEIGHT = 1;
	final static double POPULARITY_WEIGHT =1;
	
	/**
	 * constructor - creates a new OverlapCaseSimilarity object
	 */
	public asymetricCaseSimilarity()
	{}
	...
	public double getSimilarity(final Case c1, final Case c2) 
	{
		...
		double above = GENRE_WEIGHT * FeatureSimilarity.overlap(m1.getGenres(), m2.getGenres()) + 
				...
				MEAN_WEIGHT*FeatureSimilarity.asymetric(m1.getMean(),m2.getMean())+
				POPULARITY_WEIGHT*FeatureSimilarity.asymetric(m1.getPop(),m2.getPop());
